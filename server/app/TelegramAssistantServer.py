...
class TelegramAssistantServer:
    ...
    def save_chat_id(self, user_name, chat_id):
        ...
        data = TelegramSession(
            BOT=self.cfg["TELEGRAM"]["BOT"], USER_NAME=user_name, CHAT_ID=chat_id
        )
        session.merge(data)
        session.commit()
    ...
    def action(self, update: Update, context: CallbackContext) -> int:
        ...
        self.logger.info(
            "[TelegramAssistantServer] User %s start chat and wants to %s.",
            user,
            action,
        )
    ...
    def confirmation(self, update: Update, context: CallbackContext) -> int:
        ...
        if user_data["action"] == ACTION_START:
            ...
            self.logger.info(
                "[TelegramAssistantServer] TBC: User: %s / Action %s / Oasis: %s / Duration: %s",
                user,
                action,
                oasis,
                duration,
            ...
            user_data["oasis"] = oasis
            self.logger.info(
                "[TelegramAssistantServer] TBC: User: %s / Action %s / Oasis: %s",
                user,
                action,
                oasis,
            )
    ...
    def cancel(self, update: Update, _: CallbackContext) -> int:
        ...
        update.message.reply_text(
            "Tchau! Espero conversar com você em breve",
    ...
    def send_requests(self, update: Update, context: CallbackContext) -> int:
        ...
        self.logger.info(
            "[TelegramAssistantServer] Confirmed: User: %s / Action %s / Oasis: %s",
            user,
            action,
            oasis,
        )
    ...
    def command_water_tank(self, value) -> int:
        ...
        if response.status_code != 200:
            self.logger.info(
                "[TelegramAssistantServer] water-tank service connection http status code: %s!!!",
    ...
    def command_oasis(self, oasis, value) -> int:
        ...
        response = requests.post(url, data=json_msg, headers=headers)
        self.logger.info(
            "[TelegramAssistantServer] /command service response: %s", response
        )
        if response.status_code != 200:
            self.logger.info(
                "[TelegramAssistantServer] /command service connection http status code: %s!!!",
                str(response.status_code),
            )
    ...
    def run(self) -> None:
        ...
        dispatcher.add_handler(
            ConversationHandler(
                entry_points=[
                    CommandHandler("iniciar", self.begining),
                    MessageHandler(Filters.text, self.begining),
                ],
                states={
                    W_OASIS: [MessageHandler(Filters.text, self.action)],
                    W_DURATION: [MessageHandler(Filters.text, self.duration)],
                    W_CONFIRMATION: [MessageHandler(Filters.text, self.confirmation)],
                    W_BYE: [
                        MessageHandler(
                            Filters.regex("^" + GO + "$"), self.send_requests
                        ),
    ...
    def monitor(self):
        ...
        for index, result in results.iterrows():
            ...
            self.logger.info(
                "[monitor] chat_id=%s, source=%s, message=%s",
                chat_id,
                message["SOURCE"],
                message["MESSAGE"],
            )
            self.updater.bot.sendMessage(
                chat_id=int(chat_id), text=message["MESSAGE"], parse_mode=ParseMode.HTML
            )
    ...
    def send_monitor_message(message):
        ...
        if response.status_code != 200:
            logger.info(
                "[TelegramAssistantServer] send_message http status code: %s!!!",
                str(response.status_code),
            )
