...
class SupportData(DB.Model):
    ...
    def __repr__(self):
        return "<SupportData id:{} type:{} time:{} data:{}>".format(self.NODE_ID,self.TYPE, self.TIMESTAMP, self.DATA)
    ...
    def toJson(self):
        return {"TIMESTAMP": self.TIMESTAMP, "NODE_ID": self.NODE_ID, "TYPE": self.TYPE, "DATA": self.DATA}
...
class OasisHeartbeat(DB.Model):
    ...
    def __repr__(self):
        return "<OasisHeartbeat id:{} time:{} quarantine:{}>".format(
            self.NODE_ID, self.LAST_UPDATE, self.QUARANTINE
        )
    ...
    def quarantine(self):
        return (
            self.QUARANTINE
        )  # Python3: False == 0 and True == 1.  Python2.x this is not guaranteed
...
class OasisTraining(DB.Model):
    ...
    def __repr__(self):
        return "<OasisTraining id:{} value:{} data: {}>".format(
            self.NODE_ID, self.VALUE, self.DATA
        )
...
class TelegramSession(DB.Model):
    ...
    def __repr__(self):
        return "<TelegramSession bot:{} user:{} chat_id:{}>".format(
            self.BOT, self.USER_NAME, self.CHAT_ID
        )

    def toJson(self):
        return {"BOT": self.BOT, "USER_NAME": self.USER_NAME, "CHAT_ID": self.CHAT_ID}
