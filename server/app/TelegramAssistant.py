...
class TelegramAssistant(Thread):
    ...
    def transcribe(self, file_name: str, to_gs: bool, lang_code: str = "pt-BR"):
        ...
        try:
            if to_gs:
                ...
                response = self.speech_client.long_running_recognize(
    ...
    def voice_to_text(self, update: Update, context: CallbackContext) -> None:
        ...
        to_gs = self.download_and_prep(
            file_name, update.effective_message, update.effective_message.voice
        )
    ...
    def action(self, update: Update, context: CallbackContext) -> int:
        ...
        self.logger.info(
            "[VoiceAssistant] User %s start chat and wants to %s.", user, action
        )
    ...
    def confirmation(self, update: Update, context: CallbackContext) -> int:
        ...
        if user_data["action"] == ACTION_START:
            ...
            self.logger.info(
                "[VoiceAssistant] TBC: User: %s / Action %s / Oasis: %s / Duration: %s",
                user,
                action,
                oasis,
                duration,
            ...
            user_data["oasis"] = oasis
            self.logger.info(
                "[VoiceAssistant] TBC: User: %s / Action %s / Oasis: %s",
                user,
                action,
                oasis,
            )
    ...
    def send_requests(self, update: Update, context: CallbackContext) -> int:
        ...
        self.logger.info(
            "[VoiceAssistant] Confirmed: User: %s / Action %s / Oasis: %s",
            user,
            action,
            oasis,
        )
    ...
    def command_water_tank(self, value) -> int:
        ...
        if response.status_code != 200:
            self.logger.info(
                "[VoiceAssistant] water-tank service connection http status code: %s!!!",
    ...
    def command_oasis(self, oasis, value) -> int:
        ...
        if response.status_code != 200:
            self.logger.info(
                "[VoiceAssistant] /command service connection http status code: %s!!!",
                str(response.status_code),
            )
    ...
    def run(self) -> None:
        ...
        voice_handler = MessageHandler(
            Filters.voice, self.voice_to_text, run_async=True
        )
