...
srv:listen(80,function(conn)
    conn:on("receive", function(client,request)
      ...
      buf = buf.."   <label class=\"col-xs-2 text-right\">DHT Temperature &nbsp; <i class=\"wi wi-thermometer-exterior\"></i></label>"
      buf = buf.."   <div class=\"col-xs-1 text-left\">"..tempStr.."<i class=\"wi wi-celsius\"></i></div>"
      buf = buf.."  </div>"
      buf = buf.."  <div class=\"row\">"
      buf = buf.."    <label class=\"col-xs-2 text-right\">DHT Humidity &nbsp; <i class=\"wi wi-humidity\"></i></label>"
      buf = buf.."    <div class=\"col-xs-1 text-left\">"..humiStr.."</div>"
      buf = buf.."  </div>"
      buf = buf.." </div>"
      buf = buf.."</div>"
      -- calculate data
      ...
      buf = buf.." <div class=\"panel-body\">"
      buf = buf.."  <div class=\"row\">"
      buf = buf.."   <label class=\"col-xs-2 text-right\">Temperature (SMA) &nbsp; <i class=\"wi wi-thermometer-exterior\"></i></label>"
      buf = buf.."   <div class=\"col-xs-1 text-left\">"..string.format("%02.2f",TEMPERATURE_SMA).."<i class=\"wi wi-celsius\"></i></div>"
      buf = buf.."  </div>"
      buf = buf.." </div>"
      buf = buf.."</div>"
      ...
      buf = buf.."  <div class=\"panel-body\">"
      buf = buf.."   <div class=\"row\">"
      buf = buf.."    <label class=\"col-xs-2 text-right\">Temperature threshold &nbsp; <i class=\"wi wi-thermometer-exterior\"></i></label>"
      buf = buf.."    <div class=\"col-xs-1 text-left\">"
      buf = buf.."     <input type=\"number\" name=\"temp\" min=\"10\" max=\"30\" value=\""..string.format("%02d",TEMPERATURE_THRESHOLD).."\" onchange=\"form.submit()\">"
      buf = buf.."    </div>"
      buf = buf.."   </div>"
      buf = buf.."   <div class=\"row\">"
      buf = buf.."    <label class=\"col-xs-2 text-right\">Light &nbsp; <i class=\"wi wi-day-sunny\"></i></label>"
      buf = buf.."    <div class=\"col-xs-1 text-left\">"
      ...
      buf = buf.."   </div>"
      buf = buf.."   <div class=\"row\">"
      buf = buf.."    <label class=\"col-xs-2 text-right\">Fan &nbsp; <i class=\"wi wi-strong-wind\"></i></label>"
      buf = buf.."    <div class=\"col-xs-1 text-left\">"
