...
if [[ ! -z $NODE_IS_PLUGGED ]]; then
  ...
  ./luac.cross -o moi.lc moi.lua
  echo "Cleaning old binaries and confs from NODE..."
  nodemcu-uploader file remove profile.lua init.lua apps.lc moistureTest.lc config.lc nconfig.lua remote.reboot fan.on light.on  moi.lc
  echo "Restarting NODE..."
  nodemcu-uploader node restart
  sleep 5
  echo "Uploading..."
  nodemcu-uploader upload profile.lua config.lc apps.lc moistureTest.lc moi.lc init.lua
  exit 0
else
